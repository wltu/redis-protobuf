...
int AppendCommand::run(RedisModuleCtx* ctx, RedisModuleString** argv,
                       int argc) const {
  try {
    assert(ctx != nullptr);
    ...
    if (!api::key_exists(key.get(), m.type())) {
      ...
      if (RedisModule_ModuleTypeSetValue(key.get(), m.type(), msg.get()) !=
          REDISMODULE_OK) {
      ...
      // TODO: create a new message, and append to that message, then swap to
      // this message.
      len = _append(field, args.elements);
    }
...
AppendCommand::Args AppendCommand::_parse_args(RedisModuleString** argv,
                                               int argc) const {
  assert(argv != nullptr);
