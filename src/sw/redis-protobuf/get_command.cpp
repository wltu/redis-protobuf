...
int GetCommand::run(RedisModuleCtx* ctx, RedisModuleString** argv,
                    int argc) const {
  try {
    assert(ctx != nullptr);
...
GetCommand::Args GetCommand::_parse_args(RedisModuleString** argv,
                                         int argc) const {
  assert(argv != nullptr);
...
int GetCommand::_parse_opts(RedisModuleString** argv, int argc,
                            Args& args) const {
  auto idx = 2;
  while (idx < argc) {
    if (argv[idx] == nullptr) {
...
void GetCommand::_get_msg(RedisModuleCtx* ctx, const gp::Message& msg,
...
void GetCommand::_get_field(RedisModuleCtx* ctx, const ConstFieldRef& field,
                            Args::Format format) const {
...
void GetCommand::_get_array(RedisModuleCtx* ctx, const ConstFieldRef& field,
                            Args::Format format) const {
...
void GetCommand::_get_map(RedisModuleCtx* ctx, const ConstFieldRef& field,
                          Args::Format format) const {
  auto arr_size = field.size();
...
void GetCommand::_get_map_kv(RedisModuleCtx* ctx, const ConstFieldRef& field,
...
void GetCommand::_reply_with_msg(RedisModuleCtx* ctx, gp::Message& msg,
