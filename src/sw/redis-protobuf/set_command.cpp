...
int SetCommand::run(RedisModuleCtx* ctx, RedisModuleString** argv,
                    int argc) const {
  try {
...
int SetCommand::_run(RedisModuleCtx* ctx, RedisModuleString** argv,
                     int argc) const {
  assert(ctx != nullptr);
...
SetCommand::Args SetCommand::_parse_args(RedisModuleString** argv,
                                         int argc) const {
  assert(argv != nullptr);
...
int SetCommand::_parse_opts(RedisModuleString** argv, int argc,
                            Args& args) const {
  auto idx = 2;
  while (idx < argc) {
    if (argv[idx] == nullptr) {
...
void SetCommand::_create_msg(RedisModuleKey& key, const Path& path,
  ...
  if (RedisModule_ModuleTypeSetValue(&key, m.type(), msg.get()) !=
      REDISMODULE_OK) {
...
void SetCommand::_set_msg(MutableFieldRef& field, std::string_view sv) const {
  ...
  auto new_msg =
      RedisProtobuf::instance().proto_factory()->create(field.msg_type(), sv);
  assert(new_msg);
