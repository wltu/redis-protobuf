...
void ProtoFactory::load(const std::string& filename,
...
void ProtoFactory::_async_load() {
  while (!_stop_loader) {
    std::unordered_map<std::string, std::string> tasks;
    {
      ...
      _cv.wait(lock, [this]() {
        return this->_stop_loader || !(this->_tasks).empty();
      });
...
void ProtoFactory::_load(const std::string& filename,
                         const std::string& content) {
...
void ProtoFactory::_dump_to_disk(const std::string& filename,
