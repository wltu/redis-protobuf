...
class FieldRef {
 public:
  ...
  bool is_array_element() const { return _arr_idx >= 0; }

  bool is_map() const {
    return _field_desc != nullptr && _field_desc->is_map();
  }

  bool is_map_element() const { return bool(_map_key); }
  ...
  int32_t get_repeated_int32() const {
    return _msg->GetReflection()->GetRepeatedInt32(*_msg, _field_desc,
                                                   _arr_idx);
  }

  int64_t get_repeated_int64() const {
    return _msg->GetReflection()->GetRepeatedInt64(*_msg, _field_desc,
                                                   _arr_idx);
  }

  uint32_t get_repeated_uint32() const {
    return _msg->GetReflection()->GetRepeatedUInt32(*_msg, _field_desc,
                                                    _arr_idx);
  }

  uint64_t get_repeated_uint64() const {
    return _msg->GetReflection()->GetRepeatedUInt64(*_msg, _field_desc,
                                                    _arr_idx);
  }

  float get_repeated_float() const {
    return _msg->GetReflection()->GetRepeatedFloat(*_msg, _field_desc,
                                                   _arr_idx);
  }

  double get_repeated_double() const {
    return _msg->GetReflection()->GetRepeatedDouble(*_msg, _field_desc,
                                                    _arr_idx);
  }
  ...
  int get_repeated_enum() const {
    return _msg->GetReflection()->GetRepeatedEnumValue(*_msg, _field_desc,
                                                       _arr_idx);
  }

  std::string get_repeated_string() const {
    return _msg->GetReflection()->GetRepeatedString(*_msg, _field_desc,
                                                    _arr_idx);
  }

  const gp::Message& get_repeated_msg() const {
    return _msg->GetReflection()->GetRepeatedMessage(*_msg, _field_desc,
                                                     _arr_idx);
  }
 ...
 private:
  ...
  Msg* _get_sub_repeated_msg(Msg* msg, const gp::FieldDescriptor* field_desc,
                             int idx, std::true_type) {
    return &(msg->GetReflection()->GetRepeatedMessage(*msg, field_desc, idx));
  }

  Msg* _get_sub_repeated_msg(Msg* msg, const gp::FieldDescriptor* field_desc,
                             int idx, std::false_type) {
  ...
  Msg* _get_sub_repeated_msg(Msg* msg, const gp::FieldDescriptor* field_desc,
    ...
    return _get_sub_repeated_msg(msg, field_desc, idx,
                                 typename std::is_const<Msg>::type());
  }
  ...
  Msg* _get_map_msg(Msg* msg, const gp::FieldDescriptor* field_desc,
                    const gp::MapKey& key) {
    return _get_map_msg(msg, field_desc, key,
                        typename std::is_const<Msg>::type());
  }
...
template <typename Msg>
void FieldRef<Msg>::_validate_parameters(Msg* root_msg,
...
void FieldRef<Msg>::_parse_aggregate_field(const std::string& field) {
  ...
  if (is_map()) {
    ...
    auto size = _msg->GetReflection()->FieldSize(*_msg, _field_desc);
    if (_arr_idx >= size) {
      throw Error("array index is out-of-range: " + key + " : " +
                  std::to_string(size));
    }
  } else {
    throw Error("not an array or map");
...
void FieldRef<Msg>::set_repeated_msg(gp::Message& msg) {
  auto* sub_msg = _msg->GetReflection()->MutableRepeatedMessage(
      _msg, _field_desc, _arr_idx);
  sub_msg->GetReflection()->Swap(sub_msg, &msg);
}
