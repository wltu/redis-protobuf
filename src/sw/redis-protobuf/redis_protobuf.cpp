...
void RedisProtobuf::load(RedisModuleCtx* ctx, RedisModuleString** argv,
                         int argc) {
  assert(ctx != nullptr);

  if (RedisModule_Init(ctx, module_name().data(), module_version(),
  ...
  RedisModuleTypeMethods methods = {REDISMODULE_TYPE_METHOD_VERSION,
                                    _rdb_load,
                                    _rdb_save,
                                    _aof_rewrite,
                                    nullptr,
  ...
  _module_type = RedisModule_CreateDataType(ctx, type_name().data(),
                                            encoding_version(), &methods);
  if (_module_type == nullptr) {
    throw Error(std::string("failed to create ") + type_name() + " type");
  }

  _proto_factory =
      std::unique_ptr<ProtoFactory>(new ProtoFactory(options().proto_dir));
...
void RedisProtobuf::_aof_rewrite(RedisModuleIO* aof, RedisModuleString* key,
                                 void* value) {
  try {
    assert(aof != nullptr);
    ...
    RedisModule_EmitAOF(aof, "PB.SET", "sbb", key, type.data(), type.size(),
                        buf.data(), buf.size());
  } catch (const Error& e) {
