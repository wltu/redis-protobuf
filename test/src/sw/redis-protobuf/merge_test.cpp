...
void MergeTest::_run(sw::redis::Redis& r) {
  ...
  REDIS_ASSERT(
      r.command<long long>("PB.MERGE", key, "Msg", R"({"arr" : [1, 2]})") == 1,
      "failed to test pb.merge command");

  REDIS_ASSERT(r.command<long long>("PB.GET", key, "Msg", "/arr/0") == 1,
               "failed to test pb.merge command");

  REDIS_ASSERT(r.command<long long>("PB.LEN", key, "Msg", "/arr") == 2,
               "failed to test pb.merge command");
  ...
  REDIS_ASSERT(r.command<std::string>("PB.GET", key, "Msg", "/m/k2") == "v2",
               "failed to test pb.merge command");
}
